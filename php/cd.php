...
//$_GET['0'] = orginal command (cd)
//$_GET['1'] = flags
//$_GET['2'] = directory
//$_GET['3...'] = additional args, assumed to be part of directory name (but separated by spaces)

//Working directory is stored client side - check to make sure requested path exists, and write it to a javascript variable
...
$absDir = $VIRTUAL_DIR . $ROOT_DIR;
    ...
    if(!$handler) {
	    writeSpecial('cd error: no such directory "' . $ROOT_DIR .'"','error');
    }
    else {
	    $ROOT_DIR = preg_replace("/([^\/])$/" , "$1/" , $ROOT_DIR);
	    ?><script type="text/javascript">WD = '<? echo $ROOT_DIR; ?>';</script><?
    }
?>